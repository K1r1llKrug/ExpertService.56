...
// Подключение к PostgreSQL (Render.com сам добавит DATABASE_URL)
const pool = new Pool({
...
// Функции для работы с БД (аналогичны SQLite)
...
async function run(sql, params = []) {
    const result = await pool.query(sql, params);
    return { lastID: result.insertId || result.rows?.[0]?.id };
}
...
async function initDb() {
    console.log('🔄 Подключение к PostgreSQL на Render.com...');
